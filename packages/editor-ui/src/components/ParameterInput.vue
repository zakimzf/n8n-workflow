	...
	.extend({
		...
		methods: {
			...
			async loadRemoteParameterOptions () {
				...
				try {
					const options = await this.restApi().getNodeParameterOptions({name: this.node.type, version: this.node.typeVersion}, this.remoteMethod, resolvedNodeParameters, this.node.credentials);
